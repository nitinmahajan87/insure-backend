...
async def seed_data():
    async with AsyncSessionLocal() as session:
        async with session.begin():
            ...
            for c in corp_data:
                res = await session.execute(select(Corporate).where(Corporate.id == c["id"]))
                corp = res.scalars().first()
                if corp:
                    # Update new fields
                    corp.delivery_channel = c["delivery_channel"]
                    corp.base_folder = c["base_folder"]
                    corp.name = c["name"]
                else:
                    session.add(Corporate(**c))
